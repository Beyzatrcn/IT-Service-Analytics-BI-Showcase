...
SELECT
    ...
    service_owner,
    service_category,
    SUM(total_cost_usd) AS total_cost_usd_qtd,
    SUM(direct_cost_usd) AS direct_cost_usd_qtd,
    SUM(shared_cost_usd) AS shared_cost_usd_qtd,
    AVG(active_users) AS avg_active_users,
    SUM(opened_tickets) AS opened_tickets_qtd,
    SUM(resolved_tickets) AS resolved_tickets_qtd,
    ROUND(SUM(total_cost_usd) / NULLIF(AVG(active_users), 0), 2) AS avg_cost_per_user,
    ROUND(SUM(total_cost_usd) / NULLIF(SUM(resolved_tickets), 0), 2) AS avg_cost_per_ticket,
...
SELECT
    m.report_month,
    SUM(m.total_cost_usd) AS portfolio_total_cost_usd,
